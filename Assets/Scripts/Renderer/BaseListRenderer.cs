...
namespace MugMarket.Renderer
{
    ...
    where TRenderer : MonoBehaviour, ICellRenderer<TRenderValue>
    {
        ...
        public void Render(Transform parentTransform, TRenderer prefab, IEnumerable<TRenderValue> values)
        {
            foreach (var item in values.Select((value, index) => new { value, index }))
            {
                if (item.index >= objectPool.Count)
                {
                    ...
                    this.RenderCell(item.index, objectPool[item.index], item.value, true);
                }
                else
                {
                    this.RenderCell(item.index, objectPool[item.index], item.value, false);
                }
            }
        ...
        protected virtual void RenderCell(int index, TRenderer renderer, TRenderValue value, bool init)
        {
